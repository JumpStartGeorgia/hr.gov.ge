...
def scraper_main_parts
  ...
  @processed_ids = @postings_database.processed_ids
  @ids_to_process = []
...
def run_scraper(args = {})
  ...
  begin
    ...
    @scraper_log.error("Scraper stopped mid-run due to error: #{error_message}")
    @scraper_log.error('ERROR BACKTRACE:')
    e.backtrace.each do |backtrace_line|
      @scraper_log.error(backtrace_line)
    end
  end
